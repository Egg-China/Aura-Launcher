...
Aura Launcher provides localization support for multiple languages.

This document describes Aura Launcher's support status for these languages and provides a guide for contributors who want to help with Aura Launcher localization.

## Supported languages

Currently, Aura Launcher supports the following languages:
...
Aura Launcher uses language tags that conform to the IETF BCP 47 standard.
...
Aura Launcher will derive a search list from the current language tag and search for resources in order.
...
Aura Launcher requires all pull requests that update documentation and localization resources to also update the resources
...
Aura Launcher welcomes anyone to participate in translation and contribution.
...
We recommend that contributors submit a [feature request](https://github.com/Egg-China/Aura-Launcher/issues/new?template=feature.yml) before providing a new language translation,
...
If you want to add support for a new language to Aura Launcher, please start by translating [`I18N.properties`](../AuraLauncher/src/main/resources/assets/lang/I18N.properties).
The vast majority of Aura Launcher's texts are in this file, and translating it will translate the entire interface.
...
After determining the language tag, please create a file named `I18N_<localization file suffix>.properties` (e.g., `I18N_en.properties`) next to the [`I18N.properties` file](../AuraLauncher/src/main/resources/assets/lang).
...
After translating part of the file, you can [build Aura Launcher yourself](./Contributing.md#build-hmcl),
and your translations will be included in the compiled Aura Launcher.
...
and Aura Launcher will automatically switch to that language.

At this point, you can push the file to GitHub and submit a PR to Aura Launcher.
The maintainers of Aura Launcher will complete the remaining steps for you.
...
In addition to localized files, Aura Launcher also supports localizing certain text fields in JSON. Localized text in JSON uses the following format:
...
For missing resources in a certain language, Aura Launcher supports a resource fallback mechanism,
...
1. Normalize Language Codes
   ...
   Aura Launcher will try to map it to a registered tag.

   For example, Aura Launcher will replace the language code `eng` with `en`.
...
2. Map Macrolanguages to Individual Languages
   ...
   and the macrolanguage usually refers to a specific individual language, Aura Launcher will replace it with that individual language.

   For example, `zh` (Chinese) usually actually refers to `cmn` (Mandarin), so we replace the language code `zh` with `cmn`.

3. Derive Script

   If the current language tag does not specify a script, Aura Launcher will try to derive the script according to the following rules in order:
...
After refining the language code, Aura Launcher will derive a list of language tags based on this language tag.

For example, for the language tag `en-US`, Aura Launcher will refine it to `en-Latn-US` and derive the following search list:
...
For the language tag `zh-CN`, Aura Launcher will refine it to `cmn-Hans-CN` and derive the following search list:
...
Aura Launcher will merge resources according to the priority of this list; for resources that are difficult to merge (such as font files),
Aura Launcher will load the highest-priority resource found in this list.

If the current language uses a three-letter ISO 639 code, but there is also a corresponding two-letter code,
Aura Launcher will map it to the two-letter code before searching for resources.

For example, if the current environment's language tag is `eng-US`,
Aura Launcher will map it to `en-US` and then search for localization resources according to the above rules.

### Additional Rules for Chinese

Aura Launcher always adds `zh-CN` to the search list for all Chinese environments,
...
To simplify documentation maintenance, Aura Launcher uses a macro mechanism to automatically maintain parts of the documentation content.
