...
Aura Launcher 是一款開源、跨平臺的 Minecraft 啟動器，支援模組管理、遊戲客製化、遊戲自動安裝 (Forge、NeoForge、Cleanroom、Fabric、Legacy Fabric、Quilt、LiteLoader 和 OptiFine)、模組包建立、介面客製化等功能。

Aura Launcher 有著強大的跨平臺能力。它不僅支援 Windows、Linux、macOS、FreeBSD 等常見的作業系統，同時也支援 x86、ARM、RISC-V、MIPS、LoongArch 等不同的 CPU 架構。你可以使用 Aura Launcher 在不同平臺上輕鬆地遊玩 Minecraft。

如果你想要了解 Aura Launcher 對不同平臺的支援程度，請參見 [此表格](PLATFORM_zh_Hant.md)。

## 下載

Aura Launcher 目前處於公開預發佈階段，尚無穩定版本。所有建置的版本號均以 `-next` 結尾。
自動更新仍保持停用。嵌入經審核官方外掛商店信任根的建置會啟用內建商店；使用開發信任根的建置仍保持停用。

## 參與貢獻

Aura Launcher 是一個由社群驅動的開源專案，歡迎任何人參與貢獻程式碼或提出建議。

你可以透過以下方式參與 Aura Launcher 的開發：

- 透過在 GitHub 上[建立 Issue](https://github.com/Egg-China/Aura-Launcher/issues/new/choose) 來回報 Bug 或提出功能請求。
- 透過在 GitHub 上 Fork 倉庫並[提交 Pull Request](https://github.com/Egg-China/Aura-Launcher/compare) 來貢獻程式碼。

在參與貢獻前，請閱讀[貢獻指南](./Contributing_zh_Hant.md)，其中包含以下內容：

- [如何從原始碼構建並開啟 Aura Launcher](./Contributing_zh_Hant.md#構建-hmcl)
- [透過除錯選項調整 Aura Launcher 的行為](./Contributing_zh_Hant.md#除錯選項)
...
1. 當你分發該程式的修改版本時，你必須以一種合理的方式修改該程式的名稱或版本號，以示其與原始版本不同。(依據 [GPLv3, 7(c)](https://github.com/Egg-China/Aura-Launcher/blob/main/LICENSE#L372-L374))

   該程式的名稱及版本號可在 [Metadata.java](../AuraLauncher/src/main/java/org/jackhuang/hmcl/Metadata.java) 和建置設定中修改。

2. 你不得移除該程式所顯示的版權宣告。(依據 [GPLv3, 7(b)](https://github.com/Egg-China/Aura-Launcher/blob/main/LICENSE#L368-L370))
