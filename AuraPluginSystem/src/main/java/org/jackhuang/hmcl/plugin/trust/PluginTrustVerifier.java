...
public final class PluginTrustVerifier {
    ...
    public static PluginTrustVerifier loadDefault() throws IOException {
        try (InputStream input = PluginTrustVerifier.class.getResourceAsStream("/assets/aura-plugin-root.json")) {
            if (input == null) {
                throw new IOException("Embedded Aura Launcher plugin trust root is missing");
            }
            JsonElement parsed = JsonParser.parseReader(new InputStreamReader(input, StandardCharsets.UTF_8));
            if (!parsed.isJsonObject()) {
                throw new IOException("Embedded Aura Launcher plugin trust root is not an object");
            }
            ...
            if (!clock.instant().isBefore(rootExpiry)) {
                throw new IllegalArgumentException("Embedded Aura Launcher plugin trust root has expired");
            }
            return fromRoot(
                    root,
                    clock,
            ...
            throw new IOException("Embedded Aura Launcher plugin trust root is invalid", exception);
        }
    }
